...
*** Updating later:
=M-x= ~package-vc-upgrade~ =RET= ~4g~ =RET=
...
** [math] and [eqn] tags
LaTeX content on /sci/ is rendered using org's latex preview functionality.
In order for this to work, you'll need to install a LaTeX distribution and dvipng.
On Ubuntu, this should do the trick:
#+begin_src shell
sudo apt install texlive-latex-recommended texlive-fonts-recommended texlive-plain-generic dvipng
#+end_src

The default rendering settings produce pretty small images, so you may want to increase the scale:
#+begin_src emacs-lisp
...
#+begin_src shell
emacsclient -n -c -a "" -- "org-protocol://4g?url=https%3A%2F%2Fboards.4chan.org%2Fan%2Fcatalog"
#+end_src
