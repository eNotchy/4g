#+TITLE: 4g - Read image boards in Emacs Org mode
#+OPTIONS: toc:2 num:nil
#+PROPERTY: header-args :results silent

* Overview
4g (pronounced either like "fourg" or "for /g/" depending on your preference) lets you read image and discussion boards as Org documents in Emacs.
...
If you have =consult= installed, ~consult-org-headline~ is nice and you should consider binding it to a key in ~4g-quicknav-mode-map~.
...
*Please note that the format of the link types is subject to change until 4g has reached version 1.0.*
** Minor mode
...
Open ~M-x customize-group RET 4g~ or set them in your init.
...
(with-eval-after-load '4g
  (setopt
   ...
   ;; Save thumbnails on a ramdisk so they get deleted at every reboot:
   ...
   4g-videolink-command 
        (lambda (url)
          (make-process :name "videoplayer"
                        :command (list "vlc" url)
                        :noquery t))
   ;; Ask me every time something other than a video or image gets downloaded:
...
- Download dirs :: ~4g-image-download-directory~, ~4g-video-download-directory~,
  ~4g-fallback-download-directory~ — if unset or missing, 4g will prompt you for every file where to download it.
* Testing

Tests live under =test/=. Run them locally:

#+begin_src shell
emacs -Q --batch -L . \
  -l 4g.el -l test/4g-tests.el \
...
- As of now I don't have many tests, but please run them submitting:
  #+begin_src shell
...
- Try to keep byte-compile and native-compile warnings at zero.
* License
